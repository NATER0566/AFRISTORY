...
window.verifyCreator = async function(id) {
    ...
    if (res) { 
        await Swal.fire({ title: 'Approved!', text: 'Creator can now upload content.', icon: 'success', ...swalConfig });
        loadCreators(); 
        loadDashboardStats(); 
    }
};
...
window.rejectCreator = async function(id) {
    ...
    if (res) { 
        await Swal.fire({ title: 'Rejected', text: 'Creator has been removed from the queue.', icon: 'success', ...swalConfig });
        loadCreators(); 
    }
};
...
window.resolveReport = async function(id, action) {
    ...
    if (res) { 
        await Swal.fire({ title: 'Resolved', text: 'Report processed successfully.', icon: 'success', ...swalConfig });
        loadReports(); 
        loadDashboardStats(); 
    }
};
...
window.processPayout = async function(id, action) {
    ...
    if (res) { 
        await Swal.fire({ title: 'Success', text: `Payout ${action}d successfully.`, icon: 'success', ...swalConfig });
        loadPayouts(); 
        loadDashboardStats(); 
    }
};
